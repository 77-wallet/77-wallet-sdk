...
impl ObjectMetadata {
    ...
    pub fn last_modified(&self) -> Option<DateTime<Utc>> {
        ...
        let result =
            chrono::DateTime::parse_from_rfc2822(val).map(|dt| dt.with_timezone(&chrono::Utc));
        match result {
            Ok(date) => Some(date),
            Err(_e) => {
                debug!("Last modified parsed failed.{}", _e);
                None
            }
        }
    }
    ...
    pub fn expiration_time(&self) -> Option<DateTime<Utc>> {
        ...
        let result = chrono::DateTime::parse_from_rfc2822(val).map(|dt| dt.with_timezone(&Utc));
        match result {
            Ok(date) => Some(date),
            Err(_e) => {
                debug!("Expiration time parsed failed.{}", _e);
                None
            }
        }
    }
